...
class my_nanodet():
    def __init__(self, input_shape=416, prob_threshold=0.4, iou_threshold=0.3):
        ...
        if input_shape==320:
            ...
            self.net = cv2.dnn.readNet(os.path.join(base_path, 'nanodet_m.onnx'))
    ...
    def detect(self, srcimg):
        ...
        for i in range(det_bboxes.shape[0]):
            xmin, ymin, xmax, ymax = max(int((det_bboxes[i,0,0] - left) * ratiow), 0), max(int((det_bboxes[i,0,1] - top) * ratioh), 0), min(
                int((det_bboxes[i,0,2] - left) * ratiow), srcimg.shape[1]), min(int((det_bboxes[i,0,3] - top) * ratioh), srcimg.shape[0])
