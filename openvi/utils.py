...
def show_confirm(title, message, ok_callback=None, cancel_callback=None):
    ...
    with dpg.mutex():
        ...
        with dpg.window(label=title, modal=True, no_close=True) as modal_id:
            ...
            dpg.add_button(label="Ok", width=75, user_data=(modal_id, True), callback=close_then_call(callback=ok_callback))
            dpg.group(horizontal=True)
            dpg.add_button(label="Cancel", width=75, user_data=(modal_id, False), callback=close_then_call(callback=cancel_callback))
    dpg.split_frame()
...
def show_error(title, message):
    with dpg.mutex():
        ...
        with dpg.window(label=title, modal=True, no_close=True) as modal_id:
            dpg.add_text(message)
            dpg.add_button(label="Ok", width=75, user_data=(modal_id, True), callback=close_then_call())
    dpg.split_frame()
    width = dpg.get_item_width(modal_id)
    height = dpg.get_item_height(modal_id)
    dpg.set_item_pos(modal_id, [viewport_width // 2 - width // 2, viewport_height // 2 - height // 2])
